...
public class RisikokartenPanel extends JPanel {
	...
	// ImageIcon image = new
	// ImageIcon(getClass().getResource("../assets/img/button.png"));
...
//	    wenn drei Karten ausgewaehlt wurden, wird als naechstes ueberprueft, ob die kombi gueltig ist
			if (auswahlPruefen(ausgeWahlteKarten)) {
				// die entsprechenden Risikokarten zu den Buttons werden in einen neuen Array
				// geschrieben, damit sie vom Player im Server geloescht werden koennen
...
//				for (KartenButton kb : spielerKartenBtn) {
	...
	public boolean auswahlPruefen(ArrayList<KartenButton> arry) {
		// alle sind gleich
		if (arry.get(0).getKarte().getSymbol() == arry.get(1).getKarte().getSymbol()
				&& arry.get(1).getKarte().getSymbol() == arry.get(2).getKarte().getSymbol()) {
			return true;
		} // alle sind unterschiedlich
		else if (arry.get(0).getKarte().getSymbol() != arry.get(1).getKarte().getSymbol()
				&& arry.get(0).getKarte().getSymbol() != arry.get(2).getKarte().getSymbol()
				&& arry.get(1).getKarte().getSymbol() != arry.get(2).getKarte().getSymbol()) {
			return true;
		} else {
			return false;
...
//				risiko.setKarteAusgewaehlt(b.getKarte(), true);
				...
				// Methode checkt nochmal, ob jetzt drei Karten ausgewaehlt wurden und
				// verarbeitet dies
				...
				if (easterE < 2) {
					...
					// JOptionPane.showInternalMessageDialog(null, "Sorry, aber grad kannst du keine
					// Karten eintauschen",
